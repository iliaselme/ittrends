sap.ui.jsview("sapui5.dashboardDashBoardTest.view.companyCodeSubCategory", {

	getControllerName: function() {

		return "sapui5.dashboardDashBoardTest.controller.companyCodeSubCategory";
	},
	...
	createContent: function(oController) {
		...
		var inputPage = new sap.m.Page({
			id: "companyCodeSubCategoryPage",
			showNavButton: true,
			navButtonTap: [oController.eventTile, oController]
		});

		var list = new sap.m.List({
			id: "sListIdCompanyCode"
		});

		var itemTemplate = new sap.m.StandardListItem({
			title: "{groupCompanyCodeModel>docId}",
			info: "{groupCompanyCodeModel>rmwwr} {groupCompanyCodeModel>waers} ",
			description: "{groupCompanyCodeModel>archiveDate}",
			type: "Active",
			press: function(evt) {
		...
		list.bindItems("groupCompanyCodeModel>/", itemTemplate);

		var input = new sap.m.SearchField({
			placeholder: "Search By DocId",
			width: "100%",
		...
		var resultPage = new sap.m.Page({
			title: "Details",
			navButtonText: "Back",
			id: "resultCompanyCodeSubCategory"
		});
